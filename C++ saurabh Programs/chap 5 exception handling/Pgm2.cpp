...
/*
Here we have defined more than 1 catch blocks for the same try block. We can have any number of catch blocks defined after a try block. So any number of catch blocks can be associated with same try block.
But out of all catch blocks associated with same try block atmost 1 catch block out of them is executed because once an exception occurs inside  a try block the remaining statements inside the try block are skipped and control directly goes to the matching catch block. So at a time only 1 exception can be generated inside try block even if we have written multiple statements in the same try block which can generate different exceptions,
